...
//get data update
	if($action == "update"){
		$id = $_GET['id'];
		$sql = "SELECT * FROM p_proses_2 WHERE id=".$id." LIMIT 1;";
		$query = mysqli_query($Connection, $sql);
		while($row = mysqli_fetch_array($query, MYSQLI_NUM)){
			$dataDetail = $row;
		}
	...
	if($action != "update"){
		echo "
            ...
            <div class='panel-body'>
                <form action='".base_url('modul/mod_dataproses2/aksi_proses2.php?act=tambah')."' method='POST'>
                    <div class='form-group'>
                        <Label for='Proses2'>Proses 2</label>
                        <input type='text' class='form-control' id='Proses2' name='Proses2' />
                    </div>
                    <div class='form-group'>
		...
		echo "
            ...
            <div class='panel-body'>
                <form action='".base_url('modul/mod_dataproses2/aksi_proses2.php?act=update')."' method='POST'>
                    <div class='form-group'>
					...
					<div class='form-group'>
                        <Label for='Proses2'>Proses 2</label>
                        <input type='text' class='form-control' id='Proses2' name='Proses2' value='".$dataDetail[1]."' />
                    </div>
                    <div class='form-group'>
                        <Label for='Probabilitas'>Probabilitas</label>
                    ...
                    <div class='form-group'>
                        ...
                        <input type='reset' class='btn btn-default' name='Reset' value='Reset'/>
						&nbsp;
						<a href='".base_url("index.php?page=dataproses2")."' class='btn btn-default'>Kembali</a>
                    </div>
                </form>
            </div>
        </div>
	...
	foreach($data_probabilitas as $data){
		echo "
			<tr>
                ...
                <td>".$data[5]."</td>
				<td align='center'>
					<a href='".base_url("index.php?page=dataproses2&act=update&id=".$data[0])."'>
						<span class='glyphicon glyphicon-edit'></span>
						<span class='sr-only'>Update</span>
					</a>
				</td>
				<td align='center'>
					<a href='".base_url("modul/mod_dataproses2/aksi_proses2.php?act=delete&id=".$data[0])."' onclick='return confirm(\"Hapus Data ?\");'>
