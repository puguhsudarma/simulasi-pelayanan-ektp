...
switch($_GET['act']){
	case 'tambah':
		...
		$sql = "INSERT INTO p_proses_2 VALUES(
													NULL,
													".$input[0].",
													".$input[1].",
													".$input[2].",
	...
	case 'update':
		...
		$sql = "UPDATE p_proses_2 SET 	proses_2 = ".$input[1].",
	...
	case 'delete':
		$id = $_GET['id'];
		$sql = "DELETE FROM p_proses_2 WHERE id = ".$id." LIMIT 1;";
		$query = mysqli_query($Connection, $sql);
		break;
}
MysqlConnectionClose($Connection);
redirect(base_url("index.php?page=dataproses2"));
?>
